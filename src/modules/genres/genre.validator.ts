  ...
  .custom((value, helpers) => {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      return helpers.error('any.custom', {
        message: 'texts must be an object of locale -> text values',
      });
    }
    ...
    for (const [locale, text] of Object.entries(value)) {
      if (typeof locale !== 'string' || !locale.trim()) {
        return helpers.error('any.custom', {
          message: 'texts keys must be non-empty locale codes',
        });
      }

      if (typeof text !== 'string' || !text.trim()) {
        return helpers.error('any.custom', {
          message: `texts.${locale} must be a non-empty string`,
        });
      }
    ...
    if (Object.keys(entries).length === 0) {
      return helpers.error('any.custom', {
        message: 'texts must contain at least one locale entry',
      });
    }
...
const displayOrderSchema = Joi.number().integer().min(0).messages({
...
const createGenreSchema = Joi.object({
  ...
  displayOrder: displayOrderSchema
    .required()
    .messages({ 'any.required': 'displayOrder is required' }),
});
...
const updateGenreSchema = Joi.object({
  ...
  .custom((value, helpers) => {
    if (value.texts === undefined && value.displayOrder === undefined) {
      return helpers.error('any.custom', {
        message: 'At least one field must be provided to update a genre',
      });
    }
    return value;
  })
...
export function validateGenreListQuery(
  query: unknown,
  locale: string,
